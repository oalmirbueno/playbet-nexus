...
BEGIN
  ...
  _affiliate_link := _dest;

  -- The LP CTA must point to the deep affiliate URL with attribution params,
  -- not to the public LP URL. Keep the already registered page intact.
  IF _affiliate_link IS NOT NULL THEN
    IF NEW.tracking_code IS NOT NULL
      AND COALESCE(NULLIF(NEW.click_id_param_name, ''), 'sub1') <> ''
      AND position(COALESCE(NULLIF(NEW.click_id_param_name, ''), 'sub1') || '=' in _affiliate_link) = 0
    THEN
      _sep := CASE WHEN position('?' in _affiliate_link) > 0 THEN '&' ELSE '?' END;
      _affiliate_link := _affiliate_link || _sep || COALESCE(NULLIF(NEW.click_id_param_name, ''), 'sub1') || '=' || NEW.tracking_code;
    END IF;

    IF NEW.influencer_id IS NOT NULL AND position('sub2=' in _affiliate_link) = 0 THEN
      _sep := CASE WHEN position('?' in _affiliate_link) > 0 THEN '&' ELSE '?' END;
      _affiliate_link := _affiliate_link || _sep || 'sub2=' || NEW.influencer_id::text;
    END IF;

    IF NEW.campanha_id IS NOT NULL AND position('sub3=' in _affiliate_link) = 0 THEN
      _sep := CASE WHEN position('?' in _affiliate_link) > 0 THEN '&' ELSE '?' END;
      _affiliate_link := _affiliate_link || _sep || 'sub3=' || NEW.campanha_id::text;
    END IF;
  END IF;
