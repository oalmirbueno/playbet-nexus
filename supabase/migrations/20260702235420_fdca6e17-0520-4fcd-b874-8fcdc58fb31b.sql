...
WITH computed AS (
  SELECT tl.id,
         ...
         public.playbet_tracked_affiliate_url(
           COALESCE(NULLIF(tl.base_url, ''), NULLIF(lpi.affiliate_link, ''), NULLIF(tl.short_url, '')),
...
WHERE computed.id = tl.id
  AND COALESCE(CASE WHEN tl.landing_page_instance_id IS NOT NULL THEN computed.share_url ELSE computed.affiliate_url END, '') <> '';
...
UPDATE public.lp_opportunities opp
   SET destination_url = COALESCE(computed.affiliate_url, opp.destination_url),
       ...
       category = CASE WHEN NULLIF(computed.game_slug, '') IS NOT NULL THEN 'casino' ELSE opp.category END,
       badge = CASE
         WHEN upper(COALESCE(opp.badge, '')) IN ('ODD EM DESTAQUE', 'JOGO EM DESTAQUE') THEN NULL
         ELSE opp.badge
       END,
       game_thumb_url = COALESCE(NULLIF(computed.game_icon_url, ''), opp.game_thumb_url),
       metadata = COALESCE(opp.metadata, '{}'::jsonb) || jsonb_build_object(
         'game_slug', computed.game_slug,
         'game_icon_url', computed.game_icon_url,
         'image_url', computed.game_icon_url,
         'media', jsonb_build_object(
           'auto_filled', true,
           'image_url', computed.game_icon_url,
           'image_alt', COALESCE(computed.game_name, opp.title, 'Jogo'),
           'media_type', 'image',
           'source_label', 'Catálogo oficial',
           'source_url', NULL
         ),
