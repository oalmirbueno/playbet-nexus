CREATE OR REPLACE FUNCTION public.protect_tracking_event_quality()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF NEW.canonical_event_name IN ('lp_view', 'click')
     AND (
       NEW.raw_payload->>'hostname' IN ('localhost', '127.0.0.1')
       OR NEW.raw_payload->>'hostname' LIKE 'id-preview--%lovable.app'
       OR NEW.raw_payload->>'search' ILIKE '%_preview%'
       OR NEW.raw_payload->>'route' ILIKE '%_preview%'
       OR NEW.raw_payload->>'referrer' ILIKE '%_preview%'
       OR NEW.raw_payload->>'referrer' ILIKE '%/lp-opportunities%'
       OR NEW.raw_payload->>'referrer' ILIKE '%/lp-instancias%'
       OR NEW.raw_payload->>'referrer' ILIKE '%/landing-pages%'
       OR NEW.raw_payload->>'referrer' ILIKE '%__lovable_%'
     ) THEN
    NEW.status := 'invalid_internal_preview';
...
DROP TRIGGER IF EXISTS trg_protect_tracking_event_quality ON public.tracking_events;
CREATE TRIGGER trg_protect_tracking_event_quality
  BEFORE INSERT OR UPDATE ON public.tracking_events
  FOR EACH ROW
  EXECUTE FUNCTION public.protect_tracking_event_quality();
...
WHERE COALESCE(is_demo, false) = false
  ...
  AND (
    ...
    OR raw_payload->>'search' ILIKE '%_preview%'
    OR raw_payload->>'route' ILIKE '%_preview%'
    OR raw_payload->>'referrer' ILIKE '%_preview%'
