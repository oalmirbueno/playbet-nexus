...
BEGIN
  ...
  -- Only skip fully-placeholder events where BOTH fields are macro templates
  IF NEW.transaction_id IS NOT NULL AND NEW.transaction_id LIKE '{%'
     AND NEW.click_id IS NOT NULL AND NEW.click_id LIKE '{%' THEN
    RETURN NEW;
  END IF;
