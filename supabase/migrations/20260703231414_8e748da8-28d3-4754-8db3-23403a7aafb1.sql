-- Denormalize platform slug/name into landing_page_instances.hype_copy so the
-- public LP (anon) can resolve the brand kit without needing RLS access to
-- tracking_links/platform_accounts/platforms.
...
-- Backfill existing instances with platform_slug/platform_name
UPDATE public.landing_page_instances lpi
SET hype_copy = COALESCE(lpi.hype_copy, '{}'::jsonb) || jsonb_build_object(
...
LEFT JOIN public.platforms p ON p.id = pa.platform_id
WHERE tl.id = lpi.source_tracking_link_id
  AND p.slug IS NOT NULL
  AND COALESCE(lpi.hype_copy->>'platform_slug','') IS DISTINCT FROM p.slug;