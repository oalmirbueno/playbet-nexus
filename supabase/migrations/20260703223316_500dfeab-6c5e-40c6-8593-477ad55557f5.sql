...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    IF NOT _has_rules THEN
      INSERT INTO public.link_materials (tracking_link_id, influencer_id, platform_id, game_slug, game_name, format, style, status, meta)
      SELECT NEW.id, NEW.influencer_id, _platform_id, NEW.game_slug, NEW.game_name, f.format,
             CASE WHEN NULLIF(NEW.game_slug, '') IS NULL AND NULLIF(NEW.game_name, '') IS NULL THEN 'platform_lockup' ELSE 'hype_neon' END,
             'queued', jsonb_build_object('icon_url', NEW.game_icon_url, 'hype_reason', NEW.hype_reason, 'link_category', NEW.link_category, 'auto', true)
  ...
  IF NEW.landing_page_instance_id IS NOT NULL THEN
    ...
    _hype_copy := jsonb_build_object('title', _title, 'subtitle', _subtitle, 'cta_label', _cta, 'game_slug', CASE WHEN _mode = 'platform_direct' THEN NULL ELSE NEW.game_slug END, 'game_name', CASE WHEN _mode = 'platform_direct' THEN NULL ELSE NEW.game_name END, 'game_icon_url', CASE WHEN _mode = 'platform_direct' THEN NULL ELSE NEW.game_icon_url END, 'bonus_offer', jsonb_build_object('enabled', _mode <> 'platform_direct', 'title', CASE WHEN _mode = 'platform_direct' THEN NULL ELSE _bonus_title END, 'code', NULL, 'note', CASE WHEN _mode = 'platform_direct' THEN NULL ELSE 'Use no cadastro.' END, 'cta_label', _cta), 'community_cta', jsonb_build_object('enabled', _mode <> 'platform_direct', 'label', CASE WHEN _mode = 'platform_direct' THEN NULL ELSE _community_label END, 'url', NULL, 'note', NULL), 'category', _cat, 'auto', true);
    ...
    SET lp_mode = CASE WHEN COALESCE((hype_copy->>'auto')::boolean, true) THEN _mode ELSE lp_mode END,
        ...
        hype_copy = CASE WHEN COALESCE((hype_copy->>'auto')::boolean, true) THEN _hype_copy ELSE COALESCE(hype_copy, '{}'::jsonb) || jsonb_build_object('game_slug', COALESCE(hype_copy->'game_slug', _hype_copy->'game_slug'), 'game_name', COALESCE(hype_copy->'game_name', _hype_copy->'game_name'), 'game_icon_url', COALESCE(hype_copy->'game_icon_url', _hype_copy->'game_icon_url'), 'bonus_offer', COALESCE(hype_copy->'bonus_offer', _hype_copy->'bonus_offer'), 'community_cta', COALESCE(hype_copy->'community_cta', _hype_copy->'community_cta'), 'category', _cat) END,
