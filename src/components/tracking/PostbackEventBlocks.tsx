...
import { Copy, Check, CheckCircle2, AlertTriangle, Info, Settings2, Globe } from "lucide-react";
...
export default function PostbackEventBlocks({ platformName, trackingCode, influencerId, campanhaId }: Props) {
  ...
  const globalUrls = buildGlobalPostbackUrls(preset);

  const copy = (url: string, key: string, label: string) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <p className="text-xs font-semibold text-foreground">Postback Global</p>
        ...
        {showGlobal && (
          <>
            <p className="text-[10px] text-muted-foreground">
              Cole cada URL no campo correspondente em <strong>Configurações da conta → Postback Global</strong> na {preset.label}.
              Estes postbacks capturam eventos de <strong>todos os links</strong> automaticamente.
            </p>

            {globalUrls.map(({ event: evt, url }) => {
              const key = `global-${evt.raw_event_name}`;
              const isCopied = copiedIdx === key;

              return (
                <div key={key} className="border rounded-md p-2.5 space-y-1 bg-emerald-500/5 border-emerald-500/15">
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-1.5">
                      <Badge variant="outline" className="text-[9px] font-mono border-emerald-500/30 text-emerald-600">{evt.raw_event_name}</Badge>
                    ...
                    <Button
                      ...
                      onClick={() => copy(url, key, `${evt.label} (Global)`)}
                    >
      ...
      {/* ═══ PER-EVENT POSTBACK SECTION ═══ */}
      <div className="space-y-2">
        {/* Header */}
        <div className="flex items-center justify-between gap-2 mb-1">
          <div className="flex items-center gap-2">
            <p className="text-xs font-semibold text-foreground">Postbacks por Link</p>
        ...
        <p className="text-[10px] text-muted-foreground mb-1">
          Cole cada URL no painel da {preset.label}, no evento correspondente do link específico.
          {advancedMode
            ...
            <div key={idx} className={`border rounded-md p-2.5 space-y-1 ${
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                <Button
                  ...
                  onClick={() => copy(url, key, evt.label)}
