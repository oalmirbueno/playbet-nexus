...
describe("trackingUrl - Com LP", () => {
  it("buildPublicLpUrl returns LP URL with sub2 and sub3", () => {
    ...
    expect(url).toBe("https://oportunidades.playbet.app.br/i/camilly?sub2=inf-uuid&sub3=camp-uuid");
  });
  ...
  it("buildPublicLpUrl strips trailing slashes from domain", () => {
    ...
    expect(url.startsWith("https://oportunidades.playbet.app.br/i/camilly")).toBe(true);
  });

  it("buildPublicLpUrl uses the default LP domain and returns empty only when slug is missing", () => {
    expect(buildPublicLpUrl(null, "camilly", "x", "y")).toBe("https://oportunidades.playbet.app.br/i/camilly?sub2=x&sub3=y");
  ...
  it("resolveShareUrl prefers the LP URL when LP + instance exist", () => {
    ...
    expect(url).toBe("https://oportunidades.playbet.app.br/i/camilly?sub1=camilly&sub2=inf-uuid&sub3=camp-uuid");
