...
import { Plus, LayoutGrid, Table2, CalendarDays, Eye, Copy, Archive, ChevronLeft, ChevronRight, FileText } from "lucide-react";
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        ...
        <div className="flex gap-2 items-center">
          {data.length > 0 && <ExportDropdown data={data.map(({ id, tema, tipo, canal, jogo, influencer, status, data: d }) => ({ id, tema, tipo, canal, jogo, influencer, status, data: d }))} filename="conteudos-playbet" />}
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4 py-2">
            <div><label className="text-xs font-medium text-muted-foreground">Tema *</label><input className="input-field mt-1" value={editing?.tema || ""} onChange={e => setEditing(p => p ? { ...p, tema: e.target.value } : p)} /></div>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="text-xs font-medium text-muted-foreground">Tipo</label>
                <select className="select-field mt-1 w-full" value={editing?.tipo || "Reels"} onChange={e => setEditing(p => p ? { ...p, tipo: e.target.value } : p)}>
                  {tipoOptions.map(t => <option key={t}>{t}</option>)}
                </select>
              </div>
              <div><label className="text-xs font-medium text-muted-foreground">Canal</label>
                <select className="select-field mt-1 w-full" value={editing?.canal || "Instagram"} onChange={e => setEditing(p => p ? { ...p, canal: e.target.value } : p)}>
                  {canalOptions.map(c => <option key={c}>{c}</option>)}
                </select>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="text-xs font-medium text-muted-foreground">Influencer</label><input className="input-field mt-1" value={editing?.influencer || ""} onChange={e => setEditing(p => p ? { ...p, influencer: e.target.value } : p)} /></div>
              <div><label className="text-xs font-medium text-muted-foreground">Jogo</label><input className="input-field mt-1" value={editing?.jogo || ""} onChange={e => setEditing(p => p ? { ...p, jogo: e.target.value } : p)} /></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="text-xs font-medium text-muted-foreground">Data</label><input className="input-field mt-1" value={editing?.data || ""} onChange={e => setEditing(p => p ? { ...p, data: e.target.value } : p)} placeholder="DD/MM" /></div>
              <div><label className="text-xs font-medium text-muted-foreground">Prioridade</label>
                <select className="select-field mt-1 w-full" value={editing?.prioridade || "Média"} onChange={e => setEditing(p => p ? { ...p, prioridade: e.target.value as ConteudoItem["prioridade"] } : p)}>
